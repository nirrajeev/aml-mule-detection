...
geo = geo_all[['account_id', 'n_geo_txns', 'n_unique_locations',
                'location_entropy', 'geo_txn_ratio']].copy()
...
log(f"  Accounts with weight=0.3: {(train_with_weights['sample_weight'] == 0.3).sum():,}")
...
# ══════════════════════════════════════════════════════════════════════════════
log("Training LightGBM v3 — 5-fold stratified CV...")
# ══════════════════════════════════════════════════════════════════════════════
...
scores_df.to_csv(OUTPUTS / "cv_scores_v3.csv", index=False)

# ── Comparison vs v1 ──────────────────────────────────────────────────────
try:
    v1 = pd.read_csv(OUTPUTS / "cv_scores_lgbm_v1.csv")
    log(f"\n  v1 mean AUC: {v1['auc'].mean():.4f}  →  v3 mean AUC: {scores_df['auc'].mean():.4f}  "
...
best_fold_idx = int(scores_df['auc'].idxmax())
log(f"  SHAP using fold {best_fold_idx+1} (AUC={scores_df.loc[best_fold_idx,'auc']:.4f})")
explainer = shap.TreeExplainer(models[best_fold_idx])
...
feat_imp = pd.DataFrame({
    ...
    'lgbm_gain':  models[best_fold_idx].booster_.feature_importance(importance_type='gain'),
}).sort_values('shap_mean', ascending=False).reset_index(drop=True)

log(f"\n  Top 30 features (v3):")
print(feat_imp.head(30).to_string(index=False))

feat_imp.to_parquet(FEATS / "feature_importance_v3.parquet", index=False)
feat_imp.to_csv(FEATS / "feature_importance_v3.csv", index=False)
...
ax.set_title(f'Top 30 Features — LightGBM v3  (OOF AUC={oof_auc:.4f})')
plt.tight_layout()
plt.savefig(PLOTS / 'shap_v3.png', dpi=130, bbox_inches='tight')
plt.close()
...
axes[0].set_title(f'PR Curve v3 (AP={oof_ap:.4f})')
...
plt.tight_layout()
plt.savefig(PLOTS / 'pr_curve_v3.png', dpi=130, bbox_inches='tight')
plt.close()
...
oof_df['is_mule']      = y
oof_df['oof_lgbm_v3']  = oof
oof_df['oof_lgbm_v3_cal'] = oof_cal
oof_df.to_parquet(FEATS / "oof_lgbm_v3.parquet", index=False)

# Red herring analysis on v3
rh_v3 = oof_df[(oof_df.is_mule == 1) & (oof_df.oof_lgbm_v3 < 0.05)]
log(f"  Remaining low-confidence mules (prob<0.05): {len(rh_v3):,}")
...
out_path = SUBS / "submission_v3.csv"
...
log(f"""
  ...
  └──────────────────────────────────────────────────────┘
    Done. Submit: outputs/submissions/submission_v3.csv
""")